...
pub struct OpenMessage {
    ...
    hold_time: HoldTime, // Hold Time は異常系にて使用されるため、この実装では使用されない
    ...
    fn try_from(bytes: BytesMut) -> Result<Self, Self::Error> {
        ...
        let my_as_number = AutonomousSystemNumber::from(u16::from_be_bytes(
            bytes[20..22].try_into().context(format!(
                "AS 番号の bytes 表現`{:?}`から AS 番号に変換できませんでした",
                &bytes[20..22]
            ))?,
        ));
        let hold_time = HoldTime::from(u16::from_be_bytes(bytes[22..24].try_into().context(
            format!(
                "HoldTime の bytes 表現`{:?}`から HoldTime に変換できませんでした。",
                &bytes[22..24]
            ),
        )?));
        let b: [u8; 4] = bytes[24..28]
            .try_into()
            .context("Ip Addresss の octets を取得できませんでした。")?;
